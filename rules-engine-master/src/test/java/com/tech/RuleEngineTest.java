...
import com.paisabazaar.Application;
...
public class RuleEngineTest {
    ...
    private List<Rule> getListOfRules(){
        Rule rule1 = Rule.builder()
                ...
                .ruleId("1")
                .conditions("input.monthlySalary >= 50000.0 && input.cibilScore >= 500 && input.requestedLoanAmount<1500000 && $(bank.target_done) == false")
                .actions("output.setApprovalStatus(true); output.setInterestRate($(bank.interest)); output.setSanctionedPercentage(90);output.setProcessingFees(2000);output.setAccountNumber(input.accountNumber);")
                .priority(1)
        ...
        Rule rule2 = Rule.builder()
                .ruleNamespace(RuleNamespace.LOAN)
                .ruleId("2")
                .conditions("(input.monthlySalary < 50000.0 && input.cibilScore <= 300 && input.requestedLoanAmount >= 1000000) || $(bank.target_done) == true")
                .actions("output.setApprovalStatus(false); output.setInterestRate(0.0); output.setSanctionedPercentage(0.0);output.setProcessingFees(0);output.setAccountNumber(input.accountNumber);")
                .priority(2)
        ...
        Rule rule3 = Rule.builder()
                .ruleNamespace(RuleNamespace.LOAN)
                .ruleId("3")
                .conditions("input.monthlySalary >= 20000.0 && input.cibilScore >= 300 && input.cibilScore < 500 && input.requestedLoanAmount <= 1000000 && $(bank.target_done) == false")
                .actions("output.setApprovalStatus(true); output.setInterestRate($(bank.interest)); output.setSanctionedPercentage(70);output.setProcessingFees(1000);output.setAccountNumber(input.accountNumber);")
                .priority(2)
                .description("A person is eligible for car loan?")
                .build();
